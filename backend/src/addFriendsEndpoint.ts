...
export const addFriend = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    // remove any old 'Rejected' requests between these users
    await pool.query(
      `DELETE FROM Friends WHERE Status = 'Rejected' AND ((User1ID = ? AND User2ID = ?) OR (User1ID = ? AND User2ID = ?))`,
      [userId, friendId, friendId, userId]
    );

    // check if theyre already friends or pending
